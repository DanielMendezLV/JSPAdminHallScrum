	...
	<div class="container-fluid">
		...
		<div class="row-fluid">
			<div class="well well-sm">
				<div class="name-area">
					<h2 class="name">Reunión</h2>
				</div>
			</div>
			<div class="col-sm-10 justify col-sm-offset-2">
			<c:set var="num" scope="session" value="4" />
			<c:forEach var="reunion" items="${listaReunion}" begin="${pagin*num }" end="${((pagin+1)*num)-1 }">
				<div class="col-sm-5 justify ">
					<div class="columna">
						<strong>Fecha: <span>${reunion.getFecha()}</span></strong>  
						<br>
						Equipo: <h2 class="subtitle">${reunion.getEquipo()}</h2>
						Hora: <span>${reunion.getHora() }</span>
						<article class="contenido">
							<small data-toggle="modal" data-target="#elim${reunion.getIdreunion()}"> <a class="autorA" href="#"><span class="glyphicon glyphicon-remove"></span>    Eliminar</a> </small>
							<br>
						</article>
					</div>
					<div>
				...
				<!-- Modal -->
				<div class="modal fade" id="elim${reunion.getIdreunion() }"
					tabindex="-1" role="dialog"
					aria-labelledby="elim${reunion.getIdreunion()}Label">
						...
						<div class="modal-content">
							<div class="modal-header">
								...
								<h4 class="modal-title" id="myModalLabel">Desea eliminar :
									${reunion.getEquipo() } a las ${reunion.getFecha() }</h4>
							</div>
							<div class="modal-body">
								<a href="Eliminar.do?go=7&id=${reunion.getIdreunion() }"><button
			...
			<c:if test="${pagin != 0 && pagin!=null}">
				<a href="Redirect.do?go=7&pagin=${pagin-1 }"><button type="button" class="btn btn-primary"><span class="glyphicon glyphicon-arrow-left"></span></button></a>
			</c:if>
			<c:if test="${(((pagin+1)*num)) < listaReunion.size() }">
				<a href="Redirect.do?go=7&pagin=${pagin+1 }"><button type="button" class="btn btn-primary" data-dismiss="modal"><span class="glyphicon glyphicon-arrow-right"></span></button></a>
			</c:if>
