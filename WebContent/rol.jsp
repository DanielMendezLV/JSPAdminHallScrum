        ...
        <div class="panel panel-default"> 
            <div role="tabpanel">
                <ul class="nav nav-tabs" role="tablist">                
                    <li role="presentation" class="active"><a href="Redirect.do?go=1">Rol</a></li>
                ...
                <div class="tab-content">
                		<div role="tabpanel" class="tab-pane active" id="Rol">
                        ...
                        <div class="panel-body">	
							...
							<div class="col-md-10">
					            <div class="panel panel-default">
					                <div class="panel-heading">
					                    <h2 class="panel-title">Rols</h2>
					                </div>
					                <div class="panel-body">
					                    <ul class="list-unstyled" >
					                    	    	<c:forEach var="roles" items="${listadoRol}">				
					                        <li>
					                            <strong><span>${roles.getIdRol()}</span></strong>: <span >${roles.getNombre()}</span>
					                            <small><a style="color:blue"  href="#" >Eliminar</a></small>
					                        									<small data-toggle="modal" data-target="#detalleEquipo${roles.getIdRol() }">
													<a style="color:blue"  href="#" > Detalles </a>
												</small>
												<!-- Modal -->
												<div class="modal fade" id="detalleEquipo${roles.getIdRol()}" tabindex="-1" role="dialog" aria-labelledby="detalleEquipo${roles.getIdRol()}Label">
													<div class="modal-dialog" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
																<h4 class="modal-title" id="myModalLabel">${equipo.getIdEquipo() }  ${equipo.getEquipo() }</h4>
															</div>
															<div class="modal-body">
																	<h3>Usuarios del equipo</h3>
																	<c:forEach var="detEquipo" items="${equipo.getDetEquipo()}">																		
																		<small>${detEquipo.getUsuario()}</small>
																		<small>${detEquipo.getRol()}</small>
																	</c:forEach>	
																	<h3>Proyectos del equipo</h3>
																	<c:forEach var="proyect" items="${equipo.getProyectList()}">
																		<small>${proyect.getNombre()}</small>
																	</c:forEach>	
															</div>
