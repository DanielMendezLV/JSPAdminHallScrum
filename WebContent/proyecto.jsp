	...
	<div class="container-fluid">
		...
		<div class="row-fluid">
			<div class="well well-sm">
					<div class="name-area">
						<h2 class="name">Proyectos</h2>
					</div>
			</div>
			<div class="col-sm-10 justify col-sm-offset-2">
				<c:set var="num" scope="session" value="4" />
				<c:forEach var="proyect" items="${listadoProyecto}" begin="${pagin*num }" end="${((pagin+1)*num)-1 }">
					<div class="col-sm-5 justify ">
						<div class="columna">
							<h2 class="subtitulo">${proyect.getNombre()}</h2>
							<article class="contenido">
								<small data-toggle="modal" data-target="#detalleEquipo${proyect.getIdproyecto() }"> <a class="autorA" href="#"><span class="glyphicon glyphicon-info-sign"></span> Detalles </a></small>
								<small data-toggle="modal" data-target="#elim${proyect.getIdproyecto()}"> <a class="fechaA" href="#"><span class="glyphicon glyphicon-remove"></span> Eliminar</a></small>  
								<small><a class="fechaA" href="redUpdate.do?go=4&id=${proyect.getIdproyecto() }"><span class="glyphicon glyphicon-pencil"></span>Editar</a></small>
							</article>
						</div>
						<div>
							<br>
					...
					<!-- Modal -->
						<div class="modal fade" id="elim${proyect.getIdproyecto() }"
							tabindex="-1" role="dialog"
							aria-labelledby="elim${proyect.getIdproyecto()}Label">
								...
								<div class="modal-content">
									<div class="modal-header">
										...
										<h4 class="modal-title" id="myModalLabel">Desea eliminar
											: ${proyect.getIdproyecto() } ${proyect.getNombre() }</h4>
									</div>
									<div class="modal-body">
										<a href="Eliminar.do?go=4&id=${proyect.getIdproyecto() }"><button
					...
					<!-- Modal -->
						<div class="modal fade"
							id="detalleEquipo${proyect.getIdproyecto() }" tabindex="-1"
							role="dialog"
							aria-labelledby="detalleEquipo${proyect.getIdproyecto() }Label">
							<div class="modal-dialog" role="document">
								<div class="modal-content">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal"
										...
										<h4 class="modal-title" id="myModalLabel">${proyect.getIdproyecto() }
											${proyect.getNombre() }</h4>
									</div>
									<div class="modal-body">
				...
				<c:if test="${pagin != 0 && pagin!=null}">
					<a href="Redirect.do?go=4&pagin=${pagin-1 }"><button
							type="button" class="btn btn-primary"><span class="glyphicon glyphicon-arrow-left"></span></button></a>
				</c:if> <c:if test="${(((pagin+1)*num)) < listadoProyecto.size() }">
					<a href="Redirect.do?go=4&pagin=${pagin+1 }"><button
